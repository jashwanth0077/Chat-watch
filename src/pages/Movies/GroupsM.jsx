...
const Groups = () => {
  ...
  const fetchPopular = async () => {
    try {
      const popularUrl = `${apiUrl}/movies/groups/popular`;
      ...
      await Promise.all(
        popularGroups.map(async (g) => {
          ...
          try {
            const r = await fetch(
              `${apiUrl}/movies/groups/${sid}/access-status`,
              { credentials: "include" }
            );
...
const fetchMyGroups = async () => {
  try {
    ...
    // 2) hit your endpoint
    const myUrl = `${apiUrl}/movies/my-groups`;
  ...
  const requestAccess = async (id) => {
    try {
      const res = await fetch(
        `${apiUrl}/movies/groups/${id}/request-access`,
  ...
  const handleCreate = async (e) => {
    ...
    setCreating(true);
    try {
      const res = await fetch(`${apiUrl}/movies/groups`, {
        method:  "POST",
            ...
            {myGroups.length === 0 ? (
              ...
              myGroups.map((g) => (
                <div key={g.group_id} className="group-card">
                  ...
                  <p>{g.description}</p>
                  <p>Role: {g.role}</p>
                  <button onClick={() => navigate(`/movies/groups/${g.group_id}`)}>
                    Enter Group
                  </button>
                </div>
              ))
              ...
              return (
                <div key={id} className="group-card">
                  ...
                  {status === "joined" || status === "admin" || !isPrivate ? (
                    <button onClick={() => navigate(`/movies/groups/${id}`)}>
                      Enter Group
                    </button>
                  ) : status === "requested" ? (
